...
export async function getPriceHistoryForCategory(
  categoryId: string,
  days: number = 30
) {
  const supabase = await getSupabaseServerClient();
...
export async function getAveragePriceForCategory(categoryId: string) {
  ...
  return parseFloat(data?.avg_price_per_kg as string) || null;
}
...
export async function createLot(lot: Partial<Lot>) {
  ...
  const { data, error } = await supabase
    .from('lots')
    .insert([lot])
    .select('*, materials_categories(*), collector_profiles(*)')
    .single();
...
export async function getLotById(id: string) {
  ...
  if (error) throw new Error(`Failed to fetch lot: ${error.message}`);
  return data as LotWithRelations;
}

export async function updateLotStatus(
  id: string,
  status: 'draft' | 'analyzed' | 'negotiating' | 'accepted' | 'completed'
) {
  const supabase = await getSupabaseServerClient();
  const { data, error } = await supabase
    .from('lots')
...
export async function createTransaction(transaction: Partial<Transaction>) {
  ...
  const { data, error } = await supabase
    .from('transactions')
    .insert([transaction])
    .select('*, lots(*), collector_profiles(*), recyclers(*)')
    .single();
...
export async function getTransactionsByCollector(collectorId: string) {
  const supabase = await getSupabaseServerClient();
  const { data, error } = await supabase
    .from('transactions')
    .select('*, lots(*), collector_profiles(*), recyclers(*)')
...
export async function updateTransactionStatus(
  id: string,
  status: 'pending_handover' | 'completed' | 'cancelled'
) {
  const supabase = await getSupabaseServerClient();
  const { data, error } = await supabase
    .from('transactions')
...
/**
 * Get dashboard stats for a collector
 */
export async function getCollectorStats(collectorId: string) {
  const supabase = await getSupabaseServerClient();

  const [collector, lots, transactions] = await Promise.all([
    supabase
      .from('collector_profiles')
      .select('*')
      .eq('id', collectorId)
  ...
  const profile = collector.data as CollectorProfile;
  const totalLots = lots.data?.length || 0;
  const completedTransactions = transactions.data?.filter(
    (t) => t.status === 'completed'
  ).length || 0;
  const totalRevenue = transactions.data?.reduce(
    (sum, t) => sum + parseFloat(t.total_amount as string),
    0
  ) || 0;
